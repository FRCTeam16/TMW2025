    ...
    public RunDMSCommand() {
        addCommands(
                Subsystems.ledSubsystem.runOnce(() -> {
                    Subsystems.ledSubsystem.resetDMSScores();
                    Subsystems.ledSubsystem.startAMDPhase(LEDSubsystem.AMDPhase.Drivetrain);
                }),
                new RunDriveTestCommand(driveDataCollector),
                new WaitCommand(1.0),
                new RunSteerTestCommand(steerDataCollector),
                new WaitCommand(1.0),
                new DisplayDMSDataCommand(driveDataCollector, steerDataCollector)); // sets amd phase
    }
}
